...
export function logRelayBanner(libp2p: Libp2p): void {
  ...
  console.log('')
  console.log(
    'Dial one of the above from the client (include /p2p/<peerId> for remote hosts). Transports: TCP, WebSocket (cleartext WS + Noise; not WSS/AutoTLS), WebRTC-Direct (/webrtc-direct + certhash). QUIC: see README (needs libp2p v3 stack).'
  )
